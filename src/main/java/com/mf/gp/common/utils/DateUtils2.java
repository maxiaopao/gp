package com.mf.gp.common.utils;
 ...
 * 
 */
public class DateUtils2 extends org.apache.commons.lang.time.DateUtils {
	...
	private static Logger logger = Logger.getLogger(DateUtils2.class);
	...
	public static SimpleDateFormat sdfDateTime = new SimpleDateFormat(DateUtils2.DATE_FORMAT_DATETIME);
	// Global SimpleDateFormat object
	public static SimpleDateFormat sdfDateOnly = new SimpleDateFormat(DateUtils2.DATE_FORMAT_DATEONLY);
	...
	public static java.sql.Timestamp toSqlTimestamp(String sDate) {
		...
		if (sDate.length() != DateUtils2.DATE_FORMAT_DATEONLY.length()
				&&sDate.length() != DateUtils2.DATE_FORMAT_DATETIME.length()) {
			return null;
		}
		return toSqlTimestamp(sDate, 
				sDate.length() == DateUtils2.DATE_FORMAT_DATEONLY.length()
				?DateUtils2.DATE_FORMAT_DATEONLY
				:DateUtils2.DATE_FORMAT_DATETIME);
	...
	public static java.sql.Timestamp toSqlTimestamp(String sDate, String sFmt) {
		...
		if (sFmt.equals(DateUtils2.DATE_FORMAT_DATETIME)) {
			temp = sDate.replace('/', '-');
			temp = temp + ".000000000";
		} else if (sFmt.equals(DateUtils2.DATE_FORMAT_DATEONLY)) {
			temp = sDate.replace('/', '-');
	...
	public static String getSysDateTimeString() {
		return toString(new java.util.Date(System.currentTimeMillis()), DateUtils2.sdfDateTime);
	}
	...
	public static String toSqlTimestampString2(java.sql.Timestamp dt) {
		...
		String temp = toSqlTimestampString(dt, DateUtils2.DATE_FORMAT_DATETIME);
	...
	public static String toSqlTimestampString(java.sql.Timestamp dt, String sFmt) {
		...
		temp = dt.toString();
		if (sFmt.equals(DateUtils2.DATE_FORMAT_DATETIME) || // "YYYY/MM/DD
				// HH24:MI:SS"
				sFmt.equals(DateUtils2.DATE_FORMAT_DATEONLY)) { // YYYY/MM/DD
	...
	public static String timestampToStringYMD(java.sql.Timestamp timestamp) {
		SimpleDateFormat sdf = new SimpleDateFormat(DateUtils2.DATE_FORMAT_DATEONLY);
	...
	public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
		...
		Date endDate = null;
		try {
			startDate = DateUtils2.parseDate(startDateStr, DateUtils2.DATE_FORMAT_DATEONLY);
			endDate = DateUtils2.parseDate(endDateStr, DateUtils2.DATE_FORMAT_DATEONLY);
		} catch (ParseException e) {
			logger.error(e);
			return false;
		}
	...
	public static Date getMinTime(Date dt) {
		Date dt1 = null;
		dt1 = DateUtils2.getDateByStr(DateUtils2.formatDate(dt, "yyyy-MM-dd"));
		return dt1;
	}
	...
	public static List<String> getLastDays(int countDay) {
		...
		for (int i = 0; i < countDay; i++) {
			listDate.add(DateUtils2.getReqDateyyyyMMdd(DateUtils2.getDate(-i)));
		}
		return listDate;
	}
...
//		Date date3 = DateUtils2.addMinutes(new Date(), 5);
//		System.out.println(new Date().after(DateUtils2.addMinute(new Date(), -10)));
//		Date date1 = DateUtils2.addMinutes(new Date(), -1);
//		Date date2 = DateUtils2.addMinutes(date1, 5);
//		System.out.println(date1);
//		System.out.println(date2);
//		System.out.println(DateUtils2.isBetween(new Date(), date1, date2, DateUtils2.COMP_MODEL_TIME));
//
//		Integer a = 1;
//		int b = 1;
